...
final_file = "/usr/local/spark/resources/data/raw/gamma/dt="+dtStr+"/final_file.csv"
...
job_eba = PythonOperator(task_id='mergegamma', python_callable=triggerBatch, op_kwargs={"api_url":"https://api.eu-de.ae.cloud.ibm.com/v3/analytics_engines/2f641c08-2aee-438c-b8a0-eb1738f88c58/spark_applications","access_token":Variable.get("Access_Token"), "jobDetails":{"application_details": {      
        ...
        "conf": {"spark.hadoop.fs.cos.Gamma.endpoint": "s3.direct.eu-de.cloud-object-storage.appdomain.cloud",
                 ...
                 "spark.app.name": "SF-EBA-PILLAR-III"      }   }  }}, dag=dag)

job_eba_status = PythonOperator(task_id='job_eba', python_callable=jobCompletionCheck, op_kwargs={"api_url":"https://api.eu-de.ae.cloud.ibm.com/v3/analytics_engines/2f641c08-2aee-438c-b8a0-eb1738f88c58/spark_applications/{{ task_instance.xcom_pull(task_ids='job_eba') }}/state","access_token":Variable.get("Access_Token")}, dag=dag)   
