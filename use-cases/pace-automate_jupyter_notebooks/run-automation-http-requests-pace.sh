...
cd $HOME/crontab
DATENB=$(date +'%Y-%m-%d')
NBLOGFILE=$HOME/notebooks/crontab/crontab-run-jobhttprequests-pace$DATENB.log
#
rm -rf  /tmp/*
sleep 2
#
echo "Daily Job Run - http requests pace - Start Date : $DATENB" > $NBLOGFILE
#
...
$HOME/anaconda3/bin/jupyter \
	       nbconvert --to notebook --execute --allow-errors --ExecutePreprocessor.timeout=3600 \
	              $HOME/notebooks/HTTP-Requests-PACE.ipynb  \
		             --output $HOME/notebooks/HTTP-Requests-PACE-checkpoint_${DATENB}_.ipynb >> $NBLOGFILE
#
DATENB=$(date +'%Y-%m-%d')
